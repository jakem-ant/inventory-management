              ...
              <tr v-for="item in backlogItems" :key="item.id">
                ...
                <td>
                  <span class="badge danger">
                    {{ Math.abs(item.quantity_needed - item.quantity_available) }} {{ t('backlog.unitsShort') }}
                  </span>
                </td>
                <td>
                ...
                <td>
                  <!-- Keep raw priority value for the CSS class; translate only display text -->
...
export default {
  ...
  setup() {
    ...
    // Filter backlog to only items whose SKU matches currently filtered inventory.
    // When no location/category filter is active, all backlog items are shown.
